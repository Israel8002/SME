...
const PingExecutor_1 = require("./services/PingExecutor");
...
async function logEvent(nivel, modulo, evento, descripcion) {
    const timestamp = new Date().toISOString();
    console.log(`[${timestamp}] [${nivel}] [${modulo}] ${evento}: ${descripcion}`);
    try {
        await query.run(`INSERT INTO logs (fechaHora, nivel, modulo, evento, descripcion, usuario, ipEquipo, versionSistema)
       VALUES (?, ?, ?, ?, ?, 'Monitor', '127.0.0.1', '1.0.0')`, [timestamp, nivel, modulo, evento, descripcion]);
    }
    catch (err) {
        console.error("Monitor logEvent write error:", err.message);
    }
}
async function updateMonitorStatus(ipsMonitoredCount) {
    const now = new Date();
    ...
    try {
        await query.run(`UPDATE monitor_status SET
        ultimaEjecucion = ?,
        memoria = ?,
        cpu = ?,
        ipsMonitoreadas = ?,
        ultimaActualizacion = ?
       WHERE id = 1`, [now.toISOString(), parseFloat(memUsage.toFixed(2)), parseFloat(cpuLoad.toFixed(2)), ipsMonitoredCount, now.toISOString()]);
    }
    catch (err) {
        console.error("Failed to update monitor status:", err.message);
    }
}
async function generateFolio(dateStr) {
    // dateStr is YYYYMMDD
    const year = dateStr.substring(0, 4);
    // Find max sequential code for the current year
    const pattern = `SME-${year}%`;
    const maxRow = await query.get(`SELECT folio AS maxFolio FROM tickets 
     WHERE folio LIKE ? 
...
async function triggerTicketOpen(unitId, failingIpIds, reason) {
    try {
       ...
       VALUES (?, ?, ?, ?, 'Abierto', ?, 1)`, [folio, unitId, dateStr, timeStr, reason]);
        ...
        for (const ipId of failingIpIds) {
            ...
            await query.run(`INSERT INTO ticket_details (ticketId, ipId, descripcion, estadoInicial, estadoFinal)
         VALUES (?, ?, ?, 'OFFLINE', null)`, [ticketId, ipId, desc]);
        }
        await logEvent("CRITICAL", "Tickets", "Apertura Ticket", `Ticket autogenerado con Folio ${folio} para unidad ID ${unitId} por caída de enlaces críticos.`);
    }
    catch (err) {
...
async function triggerTicketClose(unitId, openTicketId) {
    try {
        ...
        // Close ticket
        await query.run(`UPDATE tickets SET 
        fechaFin = ?, 
        horaFin = ?, 
        duracionSegundos = ?, 
        estado = 'Cerrado', 
        observaciones = 'Recuperación confirmada de enlaces críticos.'
       WHERE id = ?`, [dateStr, timeStr, durationSeconds, openTicketId]);
        // Close details
        await query.run(`UPDATE ticket_details SET estadoFinal = 'ONLINE' WHERE ticketId = ?`, [openTicketId]);
        await logEvent("INFO", "Tickets", "Cierre Ticket", `Ticket Folio ${ticket?.folio || openTicketId} cerrado automáticamente tras recuperación del enlace.`);
    }
    catch (err) {
        console.error("Error closing ticket automatically:", err.message);
    }
}
async function cleanupOldHistory(retentionDays) {
    if (retentionDays <= 0)
        return;
    try {
        ...
        const result = await query.run("DELETE FROM ping_history WHERE fechaHora < ?", [limitStr]);
        if (result.changes > 0) {
            await logEvent("INFO", "Base de datos", "Depuración", `Se eliminaron ${result.changes} registros antiguos del historial de pings conforme a la política de retención (${retentionDays} días).`);
        }
    }
    catch (err) {
...
async function monitorCycle() {
    ...
    `);
        ...
        if (sondaIpRow) {
            ...
            const res = await PingExecutor_1.PingExecutor.ping(sondaIpRow.direccionIP, timeout);
            const nowISO = new Date().toISOString();
            await query.run(`INSERT INTO ping_history (ipId, fechaHora, resultado, latencia, mensajeError)
         VALUES (?, ?, ?, ?, ?)`, [sondaIpRow.id, nowISO, res.status, res.latency, res.errorMsg]);
            ...
            if (res.status === "OFFLINE") {
                ...
                if (tracker.consecutiveFailures === targetFailures && tracker.currentState !== "OFFLINE") {
                    ...
                    await logEvent("CRITICAL", "Servicio Monitor", "Caída Sonda", `Caída confirmada de la SONDA MONITOR con IP: ${sondaIpRow.direccionIP}. El escaneo de otras IPs se ha detenido para evitar tickets falsos.`);
                ...
                if (tracker.consecutiveRecoveries === targetRecoveries && tracker.currentState !== "ONLINE") {
                    ...
                    await logEvent("INFO", "Servicio Monitor", "Recuperación Sonda", `Recuperación confirmada de la SONDA MONITOR con IP: ${sondaIpRow.direccionIP}. Se reanuda el escaneo normal.`);
                    await evaluateUnitTickets(sondaIpRow.unitId);
                }
            }
        ...
        if (proceedWithAllPings) {
            ...
            const openTickets = await query.all("SELECT DISTINCT unitId FROM tickets WHERE estado = 'Abierto'");
            ...
            const pingResults = await QueueManager_1.QueueManager.runConcurrent(otherIpRows, maxConcurrency, (ip) => PingExecutor_1.PingExecutor.ping(ip.direccionIP, ip.timeout || defTimeout));
            const nowISO = new Date().toISOString();
            ...
            for (let i = 0; i < otherIpRows.length; i++) {
                ...
                // Save history row
                await query.run(`INSERT INTO ping_history (ipId, fechaHora, resultado, latencia, mensajeError)
           VALUES (?, ?, ?, ?, ?)`, [ip.id, nowISO, res.status, res.latency, res.errorMsg]);
                ...
                if (res.status === "OFFLINE") {
                    ...
                    if (tracker.consecutiveFailures === targetFailures && tracker.currentState !== "OFFLINE") {
                        tracker.currentState = "OFFLINE";
                        await logEvent("WARNING", "Servicio Monitor", "Caída Enlace", `Caída confirmada del enlace ${ip.direccionIP} (${ip.descripcion}) de la unidad ID ${ip.unitId}.`);
                    ...
                    if (tracker.consecutiveRecoveries === targetRecoveries && tracker.currentState !== "ONLINE") {
                        tracker.currentState = "ONLINE";
                        await logEvent("INFO", "Servicio Monitor", "Recuperación Enlace", `Recuperación confirmada del enlace ${ip.direccionIP} (${ip.descripcion}) de la unidad ID ${ip.unitId}.`);
                        evaluatedUnits.add(ip.unitId);
                    }
                }
            }
...
async function evaluateUnitTickets(unitId) {
    try {
        // Fetch active ticket for the unit
        const activeTicket = await query.get("SELECT id, folio FROM tickets WHERE unitId = ? AND estado = 'Abierto'", [unitId]);
        ...
        if (criticalIps.length === 0)
            return; // No critical IPs, no ticket logic
