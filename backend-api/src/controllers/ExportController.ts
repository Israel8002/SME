...
export class ExportController {
  static async exportData(req: Request, res: Response) {
    const type = req.query.type as string; // 'tickets', 'units', 'logs'
    const format = req.query.format as string; // 'pdf', 'excel', 'csv'
    ...
    try {
      ...
      if (type === "tickets") {
        ...
        data = await query.all(sql, params);
        filename = `Reporte_Tickets_${dateStr}_${timeStr}`;
      } else if (type === "units") {
        const units = await query.all<any>(`
        ...
        data = await query.all("SELECT * FROM logs ORDER BY id DESC LIMIT 500"); // Cap at 500 for export
      ...
      const logExport = async (success: boolean, filePath: string) => {
        ...
        await query.run(
          ...
          [format.toUpperCase(), today, timeNow, filePath, success ? "EXITOSO" : "FALLIDO"]
        );
        await logEvent(
          LogLevel.INFO,
          "Exportaciones",
          format.toUpperCase(),
          `Exportación de ${type} realizada exitosamente en formato ${format.toUpperCase()}.`
        );
      };
  ...
  static async getExportHistory(req: Request, res: Response) {
    try {
      const history = await query.all("SELECT * FROM exports ORDER BY id DESC");
      res.json(history);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
...
async function generateExcel(type: string, data: any[]): Promise<ExcelJS.Workbook> {
  const workbook = new ExcelJS.Workbook();
  const sheet = workbook.addWorksheet("Reporte SME");

  // Style helper
  const headerFont = { name: "Segoe UI", bold: true, color: { argb: "FFFFFF" }, size: 11 };
  const headerFill: ExcelJS.Fill = { type: "pattern", pattern: "solid", fgColor: { argb: "0B3C5D" } };
...
function generatePDF(doc: PDFKit.PDFDocument, type: string, data: any[]) {
  // Title
